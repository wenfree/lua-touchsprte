...
t['e-服务器选择-继续按扭']={0xfeaf00, "0|0|0xfeaf00,-480|-600|0xffffff,-211|-622|0xfcd741,84|-597|0xb46907",90,458,45,1292,731}
...
t['e-同意并继续6s']={0x00a5ff, "0|0|0x00a5ff,-379|-399|0xf7e1c6,-366|-406|0xd30000,-390|-388|0xd30000",90,312,53,1037,637}
t['e-同意并继续-台服']={0x12a5fe, "0|0|0x12a5fe,-16|0|0xf7e1c6,-249|-413|0xec682f,-421|-12|0x7b7062",90,369,109,899,599}
...
function _Evevnt()
	...
	elseif d('e-文明样式选择')then
	    click(1100, 659)
	    d('e-文明样式选择',true)
	elseif d("e-同意并继续",true,1,3) or d("e-同意并继续6s",true,1,3) or d("e-同意并继续-台服",true,1,3)then
...
function _init()
	...
	if d('游戏主界面-野外',false,1,2) then
        ...
        elseif _app.allimg then
            if isimgupload() then
                uploadimg()
                _开帮助()
            end
            _app.allimg = false
        elseif _app.upimg then
            -- ocrInfo()
            log("准备截图")
...
t['弹窗-城市编辑-锤子']={0x6be4ff, "0|0|0x6be4ff,-15|7|0x004b7b,-21|-8|0xaaffff",90,35,545,89,582}
t['弹窗-城市编辑-按钮']={0xc9870b, "0|0|0xc9870b,5|-6|0xf2eeea,73|-1|0xdca72b",90,1143,188,1313,232}
...
function 初始化()
    ...
    d('游戏主界面-城内')
    d("弹窗-城市编辑-锤子",true)
    d("弹窗-城市编辑-按钮",true,1,2)
    return d("弹窗-取消城市编辑",true)
end
...
t['弹窗-升级确认'] = {0x1274ba, "359|-13|0x1274ba,-35|-348|0x70fa00,-83|-332|0xb145d0",90,313,139,1000,660}
...
t['弹窗—验证图片'] = {0x1274ba, "0|0|0x1274ba,18|6|0x00bcfa,-60|-5|0x00ccff,40|-44|0xff4343",90,133,315,1284,456}
	...
	t['弹窗—验证图片-确定-失败']={0xfefffe, "0|0|0xfefffe,106|-23|0x4995f4,-89|24|0x4995f4,-263|0|0x7e7e7e,-271|-1|0xfefffe",90,400,646,941,726}
...
t['弹窗-封号警告-台服']={0x107cc0, "2|-2|0x00d0fe,-122|-193|0xf7fafb,-184|-286|0x858278,-132|-182|0xffffff,-131|-182|0x11698e",90,538,166,934,538}
t['弹窗-滑动解锁']={0x1a79ff, "-11|0|0xd1d1d1,-37|1|0x1a79ff,1|-21|0x1a79ff,407|64|0xb3b3b3",90,386,94,941,653}
t['弹窗-滑动解锁']={0x1a79ff, "0|0|0x1a79ff,-18|0|0x1a79ff,6|-409|0xffffff",90,435,37,571,734}
...
t['弹窗-装备-确定']={0x1274ba, "0|0|0x1274ba,11|-3|0x00c2fe,25|-9|0x1274ba",90,546,644,789,678}
t['弹窗-发邮件x']={0x8f7049, "0|0|0x8f7049,-2|-11|0xa98c6a,11|0|0xa88b69",90,1043,21,1095,63}
    ...
    if v then
		...
		elseif v == '弹窗-掉档' then
            if d(v)then
                ...
                local sz = require('sz')
                local json = sz.json
                arr['arr']= json.encode({ ['switch'] = 1, ['state']='掉档',['show_state'] = os.date("%Y-%m-%d %H:%M:%S") })
                arr['id']= __game.id
                _api_rok(arr)
                return '休息'
		    end
	    elseif v == '弹窗-封号警告' or v == "弹窗-封号警告-台服" then
            if d(v) then
                local arr = {}
                arr['s']='index.update'
                arr['table']='rok_account'
                local sz = require('sz')
                local json = sz.json
                arr['arr']= json.encode({ ['switch'] = 1, ['state']='封号',['show_state'] = os.date("%Y-%m-%d %H:%M:%S") }  )
                arr['id']= __game.id
                _api_rok(arr)
                uploadimg()
                return '封号'
		...
		elseif v == "弹窗—验证图片" then
			if d(v,true,1,5) then
				...
				-- 		end
						...
						if _yzmsb() then
							if d("弹窗—验证图片-确定",true)then
                                delay(8);
                                if d("弹窗—验证图片-确定-失败") then
                                    show_state("识别失败")
					...
					elseif d( "弹窗—验证图片" ,true,1,5) then
					end
					delay(1)
				end
			end
...
t['升级-升级按钮'] = { 0xc2ff,"101|2|0xc2ff,-158|2|0xfeaf00,-298|-3|0xfeb200",degree,638,381,1162,682}
    t['升级-升级按钮-雇佣']={0xfffdd0, "0|0|0xfffdd0,17|6|0xfdc600,-24|7|0xffa500",90,917,228,1140,586}
...
function _升级子程序()
    ...
    while (d("升级-升级按钮") or d("升级-前往") ) and i < 20 do
		...
		if d("升级-升级按钮")then
			...
			if not(_UI.压堡) and (dengji=='2' or dengji=='3') and not( d("升级-宝石不足") ) then
				d('升级-宝石-立即升级',true)
...
function _SignIn()
	log("签到")
	VIP次数 = VIP次数 + 1
	if VIP次数 < 3 and d("签到-vip",true,1,2)then
		...
		elseif d("签到-领取-红左",true,1,2)then
            local i=0
            while d("签到-领取-红左",true,1,2) and i< 12 do
                i=i+1
                d('签到-红点',true,1,2)
            end
		end
...
t['英雄-放大镜']={0x056599, "0|0|0x056599,-2|-49|0x3bb3e6",90,406,460,520,578}
    ...
    t['英雄-开启']={0x1274ba, "0|0|0x1274ba,-22|-11|0x00ccff",90,37,570,1315,681}
	...
	while d('采矿-放大镜') and 打野次数记数 < _UI.打野次数 and _UI.打野 do
        ...
        if tonumber(_UI.monsterlevel) == 1  then
            ...
            if dayeweizi_M and dayeweizi_M_c == getColor(dayeweizi_M[1], dayeweizi_M[2]) then
                ...
                if tonumber( _UI.monsterlevel ) > 25 then
                    _UI.monsterlevel = tonumber( _UI.monsterlevel ) - 25
                end
                
                for i = 2,tonumber( _UI.monsterlevel ) do
                    d('打野-增加开关',true,1,rd(2,6)/10)
                end
		...
		if d("采矿-搜索",true,1,1)then
		  ...
		  --  d('打野-创建部队-行军',true,1,3)
			...
			if d('打野-搜索',true)then
                ...
                if d("采矿-创建部队")  then
                    if d('打野-创建部队-选择驻扎',true,1,1) then
                        d('打野-创建部队-行军',true,1,3)
                    end
                end
            ...
            if d("采矿-搜索",false,1,3)then
                ...
                while kung_i < 4 do 
                    ...
                    elseif tonumber(_UI.采集.采矿等级) < Kuang_() then
                        d('打野-增加开关',true,3,rd(5,10)/10)
                    elseif tonumber(_UI.采集.采矿等级) > Kuang_() then
                        d('打野-增加开关',true,1,rd(5,10)/10)
                    end
                end
            end
...
t['日历-奖励界面-左侧面板-绿']={0xfffff4, "0|0|0xfffff4,10|0|0x1eff00",90,154,56,203,691}
...
t['任务-增产任务']={0x1476bb, "0|0|0x1476bb,-15|0|0x00bffd,-735|-29|0x4c2815,-780|-30|0x01eade,-782|-36|0xffffff",90,172,256,1165,679}
    t['任务-增产任务-资源类']={0xd9d7d0, "0|0|0xd9d7d0,204|17|0x8a887e,391|15|0x88867c,575|17|0x8a887e",90,163,47,1189,137}
	t['任务-增产任务-提升']={0x3be510, "0|0|0x3be510,15|8|0x3ae50c,30|10|0xca7100",90,938,165,1119,317}
	t['任务_增产类-是否使用超出-是']={0x3c2d1c, "-281|-177|0x00334e,-52|-109|0x00c2fe",90,306,416,1008,600}
	t['任务_增产类-是否使用超出-是']={0x980e0e, "0|0|0x980e0e,42|-1|0xe03d34,326|1|0x00c2ff",90,314,274,1005,582}
	t['任务_增产类-是否使用超出-是']={0x980e0e, "0|0|0x980e0e,65|2|0xdf3d34,355|-10|0x1274ba",90,316,440,1008,580}
...
t['设置-执正官资料-设置']={0xf3e9d1, "0|0|0xf3e9d1,44|36|0xdabfb9,-676|-21|0xf3d321",90,242,479,1094,643}
    t['设置-执正官资料-通用设置']={0xf6ecd8, "0|0|0xf6ecd8,22|6|0x908b9c,-245|-5|0xfcf9a9",90,239,97,853,299}
...
--联盟领地 台服优化
t['联盟采集-联盟界面-领地']={0xe92b21, "0|0|0xe92b21,-4|44|0x67ff1d",90,798,361,1167,481}
    ...
    t['联盟采集-联盟界面-领地-联盟资源中心']={0xe0a700, "0|0|0xe0a700,-7|-30|0xfeff00,30|-13|0x8c4106",90,143,187,1196,570}
        ...
        t['联盟采集-联盟界面-领地-联盟资源中心-采集中']={0x022c3d, "0|0|0x022c3d,-17|19|0x39bdf4,22|17|0x35b4e9",90,160,582,970,674}        --修正
