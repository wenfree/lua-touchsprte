...
-----------------------起手位置----------------------
ls = {}
ls['今日任务']={{1097,43,0x7f7f86},{499,50,0x000000},{37,43,0x8e8786},{496,53,0xdddddd},}
ls['对战模式']={{1099,43,0x80807a},{987,25,0x868081},{45,43,0x8d8585},{980,23,0x3c2409},}
...
ls['狂野模式']={ 0x2e1d09, 90, 1066, 34, 1103, 62}
ls['模式选择']={{667,11,0xba9156},{896,542,0x61ffff},{55,43,0x8c8484},}
ls.模式选择 = { 0x2766fc, "-3|2|0x67ffff", 80, 844, 502, 944, 587 }
...
ls['系统公告']={{292,233,0xffec7d},{274,235,0x424030},{275,271,0x413b2b},}
...
MyTable = {
    ...
    views = {
        ...
        {["type"] = "RadioGroup",["list"] = "1,2,3,4,5,不",["select"] = "0",},
...
init('0',1)
app_bid = 'com.blizzard.wtcg.cn.hearthstone'; 	--炉石传说
...
function 出牌()
	if fp(ls.牌展开,a.牌展开区,'牌展开')~=true then
		if fp(ls.绿色,a.出牌区,'有可出卡')or fp(ls.金色,a.出牌,'出牌')then
			click(x+50,y)
			delay(1.5)
		end
	end
...
function other()
	...
	elseif done(ls.我的套牌,'我的套牌')then
		delay(2)
		if f_p(ls.狂野模式,'狂野模式') and standard == 1 then
			click(1080,44)
		elseif f_p(ls.狂野模式,'狂野模式')~=true and standard ==2 then
			click(1080,44)
		end
		if deck <= 9 then
			click(53,308)
		...
		choice = true
	elseif f_pic(ls.模式选择,'模式选择')then
		if choice then
...
function surrender()
	if surrender_mun <= 4 and win >= surrender_mun then
		click(1097,23)
		click(567,153)
		win = 0
		return true
...
while (true) do
	...
	if fp(ls.金色,a.出牌区,'出牌区')or fp(ls.绿色,a.判定信息,'我的回合') then
		other_mun = 0
		出牌()
		出牌()
		出牌()
