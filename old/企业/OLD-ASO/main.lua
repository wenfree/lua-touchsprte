--[[
appstoreLogin(name,pwd) --登陆函数
appstoreLogout() --退出函数
appstoreStatus() --查询状态，登陆返回1
appstoreSearchApp(appid,max,wait)
	--AppStore搜索页面寻找指定appid的程序页面
	--appid 程序的ID  max 查询最大条数范围内 wait 每次翻页的等待时间
	--返回1代表找到，其他值都是未找到

--appstoreSearchApp(1096003203,2200,2)
--
local tsld = loadTSLibrary("aso.tsl") --库加载
if tsld.status ~= 1 then --验证判断
    dialog("插件加载异常", 0)
    return
end
require("ascmd") --需要加载
...
m.有结果={ 0x0080fc, "45|-21|0x0080fc,89|1|0x0080fc,44|29|0x0080fc", 90, 489, 148, 622, 998}
...
m.有应用={ 0x0080fc, 90, 469, 175, 630, 293}
	...
	while os.time()-outTime < 60 * 5 do
		...
		elseif setp == 3 then
			...
			elseif c_pic(m.创建昵称,'创建昵称')then
				name = myRand(7,math.random(4,8),1)..myRand(7,1,2)..myRand(3,math.random(1,3),math.random(1,2))
...
function search(keywords,image,app_id)
	...
	while (true) do
		...
		elseif search_setp == 3 then
			if move_fast == 0 then
				move_out = os.time()
				while os.time()- move_out < (rank/4.5 - 15) do
					if AppStore == frontAppBid() then
			...
			if c_tu(image)then
				...
				if active(AppStore)then
					...
					if f_pic(m.有应用图版,'有应用图版')then
						line_out = line_out + 1
						if line_out >= rank * 3 then
							closeX(AppStore)
							return false
						end
						bottom = 0
		...
		log('第步'..search_setp..'骤->'..search_arr[search_setp])
		---------------------------------超时验证-----------------------
		sotp = sotp or 0
...
while (true) do
	if GetTask()then
		--
		for im,vm in ipairs(data.tasks)do
			image = vm.image
			nLog(image)
			
			web_pic = "http://luawarehouse.applinzi.com/AsoPic/"..image..".png"
			loc_pic = "/User/Media/TouchSpriteENT/res/"..image..".png"
		--	loc_pic = "/User/Media/TouchSprite/res/"..image..".png"
			
			if file_exists(loc_pic)then
				nLog("图片已存在")
			else
				nLog(downFile(web_pic,loc_pic))
			end
			...
			for i,v in ipairs(data.tasks)do
				...
				if search(keywords,image,app_id)then
					if open_comments then
						if Comment(v.comment.title,v.comment.content)then
