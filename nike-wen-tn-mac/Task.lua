			...
			if string.find(recursiveDescription,"SNKRS_inhouse.SplashLottieView")==nil then 
				...
				if session then 
					...
					elseif  session == "waitPhone" then 
						...
						if info.phone == "" then info.phone = DMGetPhone() SaveInfo(info) log(info.phone) end 
					...
					elseif  session == "page3" then
						log(info.email) showWebview("电子邮件","检查异常:"..repeatTime) sys.msleep(500)
