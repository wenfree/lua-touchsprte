...
function _init()
	...
	if d('游戏主界面-城内',false,2) then
        moveTo(1000+rd(10,100),100,100,300+rd(50,100),30)
        delay(0.5)
        moveTo(1000+rd(10,100),100,100,300+rd(50,100),30)
        -- delay(0.5)
        -- moveTo(1000+rd(10,100),300,100,300,30)
        delay(1)
        d('游戏主界面-回到城内',true,1,3)
        if d('游戏主界面-回到城内-白天定位') then
            return true
        end
    end
end
...
function _Collection()
	log("采集")
	while _UI.采集.key  and 采矿次数 <= 3 do
	    采矿次数 = 采矿次数 + 1
...
function read_mail()
    ...
    if d('邮件-邮件',true,1,4)then
        ...
        local 派遣次数 = 0
        while d('邮件-邮箱界面')  and _UI.读邮件 and os.time() - mailtime < 30 do
	...
	if d('任务_点击任务',true,1,3)then
		...
		elseif d( '任务_点击任务_前往' ,true,1,3)then
		    if d('任务_点击任务_科技面板') then
    			local i=0
    			local 查一下是是否要升级军事 = true
    			t['任务_点击任务_科技面板-军事未点']={0x417182, "0|0|0x417182,54|1|0x427384,-3|-21|0xdafbff",90,269,373,432,458}
    			t['任务_点击任务_科技面板-军事点好']={0x005c95, "0|0|0x005c95,54|1|0x00578c,-3|-21|0xdafbff",90,269,373,432,458}
    			t['任务_点击任务_科技面板-可升科技']={0x3d6e7e, "0|0|0x3d6e7e,10|-3|0x3a6979,12|3|0x427384",90,156,118,1182,682}
    			while d('任务_点击任务_科技面板') and i< 3 do
    			    if 查一下是是否要升级军事 and d('任务_点击任务_科技面板-军事',true,1) then
    			    elseif d('任务_点击任务_科技面板-军事点好') then
    			        click(94, 192)
    			        查一下是是否要升级军事 = false
    			    elseif d('任务_点击任务_科技面板-军事未点',true) then
    			    end
    			    
    			    if d('箭头向下',true,1,2) then
    			        click(x,y+50)
    			    end
    			    d('任务_点击任务_科技面板-可升科技',true,1,2)
